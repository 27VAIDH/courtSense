...
interface WinMethodDistributionProps {
  rallyAnalyses: RallyAnalysis[]
}

const MIN_ANALYSES = 5

const WIN_METHOD_COLORS: Record<string, string> = {
  'Winners / Kill shots': '#FF6D00', // accent orange
  'Opponent errors': '#448AFF', // secondary blue
  'Long rallies / Outlasted them': '#00E676', // primary green
  'Serve dominance': '#7C4DFF', // purple
  'Drop shots / Deception': '#FFD600', // tight yellow
}

export default function WinMethodDistribution({
  rallyAnalyses,
}: WinMethodDistributionProps) {
  const stats = useMemo(() => {
    const methodCounts: Record<string, number> = {}

    for (const ra of rallyAnalyses) {
      if (ra.winMethod) {
        methodCounts[ra.winMethod] = (methodCounts[ra.winMethod] || 0) + 1
      }
    }
            ...
            <PieChart>
              ...
              >
                {stats.chartData.map((entry, index) => (
                  <Cell
                    key={index}
                    fill={WIN_METHOD_COLORS[entry.name] || '#B0B0B0'}
                  />
                ))}
              </Pie>
            </PieChart>
          ...
          {stats.chartData.map((item) => (
            <div key={item.name} className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: WIN_METHOD_COLORS[item.name] || '#B0B0B0' }}
              />
